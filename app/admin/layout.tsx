...
import {  LayoutDashboard, Image as ImageIcon, PenTool,  ChevronRight, BookImageIcon } from 'lucide-react'; 
   ...
   return (  
    ...
    {/* SIDEBAR */} 
     ...
     <div className="mb-10 px-2"> 
         <h1 className="text-white font-display text-xl tracking-widerer uppercase">  
            GreenInk <span className="text-white ml-2">Tattoo</span>  
            </h1>  
            </div>   
            <nav className="flex-1 space-y-2">  
                ...
                <ChevronRight size={14} className="ml-auto opacity-0 group-hover:opacity-100" />  </Link>  
                ...
                <PenTool size={20} /> <span>Journal/Blog</span>  <ChevronRight size={14} className="ml-auto opacity-0 group-hover:opacity-100" />  </Link>  <Link href="/admin/bookings" className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 hover:text-white rounded-xl transition-all group">  <BookImageIcon size={20} /> <span>Bookings</span>  <ChevronRight size={14} className="ml-auto opacity-0 group-hover:opacity-100" />  </Link>  </nav>   <LogoutButton />  </aside>   {/* MAIN CONTENT - Only uses ml-64 when sidebar exists */}  <main className="flex-1 ml-64 p-12">  {children}  </main>  </div>  ); }